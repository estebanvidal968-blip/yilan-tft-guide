...
// 国服 meta 校准库（data/cn-meta.json）：提供「数据口径」标注与最后更新日期。
// 用于首页 / 详情页展示「国服实测（非全球服 OP.GG）」口径，纯服务端读取。
export function loadCnMeta() {
  try {
    const p = path.join(DATA_DIR, 'cn-meta.json');
    if (!fs.existsSync(p)) return null;
    return JSON.parse(fs.readFileSync(p, 'utf8'));
  } catch (e) {
    console.warn('[data] 读取 cn-meta.json 失败：', e.message);
    return null;
  }
}
...
// 攻略封面用的官方图标总表：装备(140) + 符文(283) + 棋子(55)，构建期一次性读入。
// key 形如 'item:死亡之刃' / 'aug:珍藏财宝 III' / 'champ:德莱文'。
// 查不到就返回空串，封面会回落到品类几何符号，不会出破图。
const CHAMP_ALIAS = {
  德莱文: '荣耀行刑官',
  阿狸: '九尾妖狐',
  厄斐琉斯: '残月之肃',
...
export function loadGuideIcons() {
  const map = {};
  const read = (file, prefix) => {
    try {
      const arr = JSON.parse(fs.readFileSync(path.join(DATA_DIR, 'tft', file), 'utf8'));
      arr.forEach((it) => {
        if (it && it.name && it.icon) map[`${prefix}:${it.name}`] = it.icon;
      });
    } catch (e) {
      console.warn(`[data] 读取 tft/${file} 封面图标失败：`, e.message);
    }
  };
  read('items.json', 'item');
  read('augments.json', 'aug');
  read('champs.json', 'champ');
  // 棋子补录短名：攻略正文写玩家习惯的「德莱文」，而 champs.json 存的是称号「荣耀行刑官」
  try {
    const champs = JSON.parse(fs.readFileSync(path.join(DATA_DIR, 'tft', 'champs.json'), 'utf8'));
    Object.entries(CHAMP_ALIAS).forEach(([short, full]) => {
      const c = champs.find((x) => x.name === full);
      if (c && c.icon) map[`champ:${short}`] = c.icon;
    });
  } catch (e) {
    console.warn('[data] 棋子短名图标补录失败：', e.message);
  }
  return map;
}
