// 腾讯混元生成层。
// 支持两套鉴权，按环境变量自动选择（OpenAI 兼容优先，其次 TC3 直连）：
//
//   ① OpenAI 兼容（推荐，Bearer Token，sk- 开头）—— 腾讯混元官方 OpenAI 接口
//      HUNYUAN_OPENAI_API_KEY   -> Authorization: Bearer <token>
//      HUNYUAN_OPENAI_BASE_URL  -> 默认 https://api.hunyuan.cloud.tencent.com/v1
//      HUNYUAN_OPENAI_MODEL     -> 默认 hy4
//   ② 腾讯云 TC3-HMAC-SHA256 直连（原生 SDK 方式）
//      HUNYUAN_SECRET_ID / HUNYUAN_SECRET_KEY / HUNYUAN_MODEL（默认 hunyuan-pro）
//
// 两套密钥均只来自服务端环境变量，前端绝不接触。
...
function getOpenAIConfig() {
  const apiKey = process.env.HUNYUAN_OPENAI_API_KEY;
  if (!apiKey) return null;
  return {
    apiKey,
    baseURL: (process.env.HUNYUAN_OPENAI_BASE_URL || 'https://api.hunyuan.cloud.tencent.com/v1').replace(/\/+$/, ''),
    model: process.env.HUNYUAN_OPENAI_MODEL || 'hy4-preview',
  };
}
...
// 统一入口：OpenAI 兼容优先，其次 TC3，再交给调用方降级模板。
async function callHunyuan(prompt) {
  // ① OpenAI 兼容（Bearer）
  try {
    const t = await callOpenAI(prompt);
    if (t) return t;
  } catch (e) {
