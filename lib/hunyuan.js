// 腾讯混元生成层（零依赖，TC3-HMAC-SHA256 直连）。
// 密钥仅来自服务端环境变量，前端绝不接触：
//   HUNYUAN_SECRET_ID / HUNYUAN_SECRET_KEY / HUNYUAN_MODEL（默认 hunyuan-pro）
// 缺密钥或调用失败时，降级为基于 stat 的模板评语，保证同步流水线可跑通。
...
export async function generateComment(comp) {
  ...
  const model = process.env.HUNYUAN_MODEL || 'hunyuan-pro';

  if (!id || !key) {
    return fallbackComment(comp);
  }
  ...
  try {
    ...
    console.warn('[hunyuan] 调用失败，降级模板评语：', e.message);
    return fallbackComment(comp);
  }
}
...
export async function generateRuneGuide(comp) {
  ...
  const key = process.env.HUNYUAN_SECRET_KEY;
  const model = process.env.HUNYUAN_MODEL || 'hunyuan-pro';
  if (!id || !key) return fallbackRuneGuide(comp);
  ...
  try {
    ...
    const text = json?.Response?.Choices?.[0]?.Message?.Content;
    if (text) return text.trim();
    return fallbackRuneGuide(comp);
  } catch (e) {
    console.warn('[hunyuan] runeGuide 调用失败，降级模板：', e.message);
    return fallbackRuneGuide(comp);
  }
}
...
export async function generatePickTips(comp) {
  ...
  const key = process.env.HUNYUAN_SECRET_KEY;
  const model = process.env.HUNYUAN_MODEL || 'hunyuan-pro';
  if (!id || !key) return fallbackPickTips(comp);
  ...
  try {
    ...
    const text = json?.Response?.Choices?.[0]?.Message?.Content;
    if (text) return text.trim();
    return fallbackPickTips(comp);
  } catch (e) {
    console.warn('[hunyuan] pickTips 调用失败，降级模板：', e.message);
    return fallbackPickTips(comp);
  }
}
